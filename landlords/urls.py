"""
landlords/urls.py
Production-ready URL configuration for the landlord dashboard.

Key fixes:
- Removed duplicate `properties/<int:pk>/edit/` route that pointed to two
  different views (PropertyUpdateView AND PropertyEditView) — kept the one
  named 'property_edit' pointing to PropertyEditView (the more complete view).
  If PropertyUpdateView is also needed, give it a distinct name/path.
- Grouped routes by feature area with comments for readability.
- login_required wrappers on community views are kept; consider using
  LoginRequiredMixin on the CBVs themselves for consistency.
"""
