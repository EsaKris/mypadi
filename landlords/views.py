...
from .models import (
    ...
    LeaseAgreement, MaintenanceRequest, Payment, Expense,
    Conversation, Message, CommunityPost, CommunityReply
)
from .forms import (
    LandlordProfileForm, PropertyForm, TenantForm, RentalApplicationForm,
    LeaseAgreementForm, MaintenanceRequestForm, PaymentForm, ExpenseForm,
    CommunityPostForm, CommunityReplyForm
)
...
class PropertyListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            ...
            .prefetch_related('leases', 'images', 'amenities')
            .order_by('-created_at')
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        all_props = list(user.owned_properties.prefetch_related('leases'))
        total = len(all_props)
        occupied = sum(1 for p in all_props if p.is_occupied)

        context.update({
            'total_properties': total,
            'occupied_count': occupied,
            'vacant_count': total - occupied,
            'active_listings': user.owned_properties.filter(is_active=True).count(),
            'total_views': sum(p.views for p in all_props),
...
# ============================================================
# Property Detail (landlord's own view)
# ============================================================
...
class PropertyCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        # Set landlord before saving
        form.instance.landlord = self.request.user

        # Force is_published=True regardless of checkbox state.
        # HTML checkboxes are not sent when unchecked, so
        # cleaned_data may contain False even though the landlord
        # didn't explicitly choose to hide the property.
        ...
        # Guarantee published via direct SQL — nothing can override this
        Property.objects.filter(pk=property_instance.pk).update(is_published=True)

        # ── Save uploaded images ─────────────────────────────────────
        # Read from request.FILES directly — most reliable approach.
        # PropertyForm.save() also attempts this via cleaned_data but
        # can silently fail if MultipleFileField validation is strict.
        # Checking .images.exists() prevents duplicates if form already saved them.
        images = self.request.FILES.getlist('images')
        # Fallback only — runs if PropertyForm.save() failed for any reason
        if not property_instance.images.exists():
            images = self.request.FILES.getlist('images')
            first_image = True
            for img_file in images:
                try:
                    ...
                    _log.error(
                        "Failed to save image '%s' for property %s: %s",
                        img_file.name, property_instance.pk, exc
                    )
        elif images and property_instance.images.exists():
            # Form already saved some images — append any that are missing
            existing_names = set(
                property_instance.images.values_list('image', flat=True)
            )
            make_primary = not property_instance.images.filter(is_primary=True).exists()
            for img_file in images:
                try:
                    PropertyImage.objects.create(
                        property=property_instance,
        ...
        count = property_instance.images.count()
        msg = (
            f'Property published with {count} image{"s" if count != 1 else ""}!'
            if count else
            'Property published to marketplace!'
        )
...
class PropertyEditView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        # ── FIX: preserve is_published on edit ───────────────────────
        # HTML checkboxes are NOT included in POST when unchecked.
        # Without this guard, every edit where the checkbox happens to
        # be unchecked (or not rendered) would set is_published=False
        # and silently hide the property from the marketplace.
        #
        # Logic:
        #   - Checkbox present in POST → landlord explicitly set it, respect it
        #   - Checkbox absent from POST → keep the current DB value unchanged
        if 'is_published' not in self.request.POST:
            current_value = self.get_object().is_published
            form.instance.is_published = current_value
            form.cleaned_data['is_published'] = current_value
        ...
        images = self.request.FILES.getlist('images')
        if images:
            make_first_primary = not property_instance.images.filter(is_primary=True).exists()
            for img_file in images:
                try:
                    PropertyImage.objects.create(
                        property=property_instance,
                        image=img_file,
                        is_primary=make_first_primary,
                    )
        ...
        # ── Delete images the landlord marked for removal ────────────
        delete_ids = self.request.POST.getlist('delete_images')
        if delete_ids:
            to_delete = property_instance.images.filter(pk__in=delete_ids)
            for img in to_delete:
                try:
                    img.image.delete(save=False)
                    img.delete()
                except Exception as exc:
                    _log.error("Failed to delete PropertyImage %s: %s", img.pk, exc)

            # Promote next image to primary if the primary was deleted
            if not property_instance.images.filter(is_primary=True).exists():
                first = property_instance.images.first()
                if first:
                    PropertyImage.objects.filter(pk=first.pk).update(is_primary=True)
...
# ============================================================
# Property Update (toggle publish / quick update)
# ============================================================
...
class PropertyUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        # Same checkbox preservation fix as PropertyEditView
        if 'is_published' not in self.request.POST:
            current_value = self.get_object().is_published
            form.instance.is_published = current_value
            form.cleaned_data['is_published'] = current_value
...
class TenantCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_invalid(self, form):
        _log.debug("TenantCreateView form errors: %s", form.errors)
...
class LeaseAgreementCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        tenant = get_object_or_404(
            Tenant, pk=self.kwargs.get('tenant_id'),
            property__landlord=self.request.user
        )
        return {
            'tenant': tenant,
...
class ExpenseCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_initial(self):
        property = get_object_or_404(
            Property, pk=self.kwargs.get('property_id'),
            landlord=self.request.user
        )
...
class ProfileEditView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self):
        ...
        if created:
            profile.phone_number = getattr(self.request.user, 'phone_number', '')
            profile.save()
        return profile
...
# ============================================================
# Message helpers (function views)
# ============================================================
...
def new_conversation(request):
    ...
    if request.method == 'POST':
        ...
        participant = get_object_or_404(UserModel, id=participant_id)

        conversation = Conversation.objects.filter(
            property=property, participants=request.user
        ...
        return redirect(f"{reverse('landlords:messages')}?conversation={conversation.id}")
...
class PostDetailView(LoginRequiredMixin, DetailView, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'replies': post.landlords_replies.all().order_by('created_at'),
...
class CommunityReplyView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.post_obj = get_object_or_404(CommunityPost, pk=self.kwargs['post_id'])
        if not (self.post_obj.visibility == 'all' or hasattr(request.user, 'landlord_profile')):
