...
class PropertyListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        # prefetch 'images' so property.primary_image never fires N+1 queries
        # across the list page (one query per card without this)
        return (
    ...
    def get_context_data(self, **kwargs):
        ...
        # Count against the FULL queryset, not just the current page slice.
        # Without this, occupied_count is wrong on page 2+.
...
class PropertyEditView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        # Scope to this landlord's properties only, prefetch images for the UI
        return (
    ...
    def form_valid(self, form):
        ...
        # ── 1. Append newly-uploaded images ────────────────────────
        #
        # CRITICAL NAME MATCH: the file input in edit.html must be named
        # "images" (not "property_images") for this getlist call to work.
        # If the input uses a different name, change it to match here.
        #
        images = self.request.FILES.getlist('images')

        # If there's currently no primary image, make the first new upload primary
        ...
        # ── 2. Delete images the user marked for removal ─────────────
        # The template sends `<input type="hidden" name="delete_images" value="<id>">`
        # for each image the landlord ticked for deletion.
        delete_ids = self.request.POST.getlist('delete_images')
        if delete_ids:
            to_delete = property_instance.images.filter(pk__in=delete_ids)
            for img in to_delete:
                try:
                    img.image.delete(save=False)   # remove file from disk/storage
                    img.delete()
                except Exception as exc:
                    _log.error("Failed to delete PropertyImage %s: %s", img.pk, exc)

            # If the primary image was deleted, promote the next available one
